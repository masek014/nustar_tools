...
class TrackerCorrelator():
    ...
    def correlate_trackers(self, window_size=60, time_step=1,
        ...
        # Ordered xx, xy, yx, yy.
        self.times = []
        self.matrices = {'xx': [], 'xy': [], 'yx': [],
                         'yy': []}  # Holds the covariance matrices
        self.covs = {'xx': [], 'xy': [], 'yx': [],
                     'yy': []}  # Holds the covariance values
    ...
    def compute_amplitude_hist(self, bin_width=0.001):
        ...
        num_bins = int((mtools.np.nanmax(self.amplitude) -
                       mtools.np.nanmin(self.amplitude))/bin_width)
        # num_bins = 100
    ...
    def fill_time(self, ax, where):
        ...
        ylim = ax.get_ylim()
        ax.fill_between(self.times, *ylim, where=where,
    ...
    def make_overview(self, time_range=None):
        ...
        for i in range(2):
            ...
            self.plot_tracker(str(i+1), xax, yax, where,
                              xlim=xlim, xticklabels=[])
