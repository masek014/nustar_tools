...
NU_DATA_DIR = pathlib.Path.cwd() / 'data'
TMP_DIR = NU_DATA_DIR / 'tar_temp/'
TAR_DIR = pathlib.Path.cwd() / 'tars'  # The input tar directory
# The directory for tars that have been untarred.
TAR_ARCHIVE = NU_DATA_DIR / 'tar_archive'
...
def move_and_untar(tar_file):
    ...
    try:
        p = parse.parse(TAR_FORMAT, tar_file)
        date = p['date']
    except:
    ...
    if os.path.isfile(TAR_ARCHIVE / dest[utilities.find_nth(dest, '/', dest.count('/'))+1:]):
        new_name = str(dest).split(
            '.tar')[0] + '_' + datetime.datetime.now().strftime('%Y%m%d_%H_%M_%S') + '.tar'
        dest = pathlib.Path(new_name)
...
def run_pipeline(id_dir: str):

    cmd = f'./run_pipe_solar.sh {id_dir}'
    os.system('heainit')
    os.system(cmd)
