...
def calculate_lightcurve_rates(evt_data, hk_file, number_frames,
    ...
    time_edges, counts, counts_err = calculate_lightcurve(
        evt_data, number_frames, time_range, energy_band, density)
...
def make_lightcurve_plot(
    ...
    if ax is None:
        plt.style.use(os.path.dirname(os.path.realpath(
            __file__)) + '/styles/lightcurve.mplstyle')
        fig, ax = plt.subplots()
    ...
    dt_times = [time_tools.nustar_to_datetime(
    ...
    if values_err is not None:
        ...
        dt_times = [time_tools.nustar_to_datetime(
            t) for t in time_edges]
        ax.fill_between(dt_times[:-1], values-values_err, values +
                        values_err, step='post', color=color, alpha=0.15)

    # Configure the title.
    start_yyyymmdd, start_hhmmss = dt_times[0].strftime(
        '%Y-%m-%d %H:%M:%S').split(' ')
    if b_title_add_date:
        title += ' - ' + start_yyyymmdd
    if b_title_add_time:
        title += ', ' + start_hhmmss
    ax.set_title(title)
...
def make_event_macropixel_lightcurves(event, fig_dir='', file_name=''):
    ...
    fig, ax = plt.subplots(nrows=num_rows, ncols=num_cols,
                           figsize=(12*num_rows, 12*num_cols))
    ...
    start_dt = datetime.datetime.strptime(
        event.start, '%Y-%m-%d %H:%M:%S').replace(tzinfo=datetime.timezone.utc)
    end_dt = datetime.datetime.strptime(
        event.end, '%Y-%m-%d %H:%M:%S').replace(tzinfo=datetime.timezone.utc)
    ...
    for pair in coord_pairs:
        ...
        time_padding = 120
        x_min = start_dt - datetime.timedelta(time_padding)
        x_max = end_dt + datetime.timedelta(time_padding)
...
def make_observation_lightcurve(evt_file, hk_file, frame_length, energy_range,
    ...
    if axes_position:
        ...
        fig, ax, rates_line = make_lightcurve_plot(
            time_edges, count_rates, count_rates_err, None, ax, color='gray')
        x_min = time_tools.nustar_to_datetime(time_edges[0])
        x_max = time_tools.nustar_to_datetime(time_edges[-1])
    ...
    #     start_dt = time_tools.nustar_to_datetime(pair[0])
    #     end_dt = time_tools.nustar_to_datetime(pair[1])
    ...
    time_range = (
        time_tools.nustar_to_astropy(
            time_edges[0]).strftime('%Y-%m-%d %H:%M:%S'),
        time_tools.nustar_to_astropy(
            time_edges[-1]).strftime('%Y-%m-%d %H:%M:%S')
    )
    avg_livetime = round(livetime.compute_average_livetime(
        hk_file, time_range)*100, 2)
    yyyymmdd = time_tools.nustar_to_astropy(time_edges[0]).strftime('%Y-%m-%d')
...
def generate_lightcurves(id_dir, frame_length=10, energy_range=(2.5, 10), axes_position=[]):
    ...
    for fpm in ['A', 'B']:
        evt_file = utilities.EVT_FILE_PATH_FORMAT.format(
            id_dir=id_dir, id_num=obs_id, fpm=fpm)
        hk_file = utilities.HK_FILE_PATH_FORMAT.format(
            id_dir=id_dir, id_num=obs_id, fpm=fpm)
