'''
Defines a function for deconvolving a NuSTAR map, taking into account the PSF
based on the optical axis and X-ray source position.
'''
...
def deconvolve(
    nustar_map: sunpy.map.GenericMap,
    psf_file: str,
    source_position: tuple[u.Quantity, u.Quantity],
    oa_position: tuple[u.Quantity, u.Quantity],
    iterations: int = 10,
    clip=False
):
    '''
    ...
    oa_position : tuple of astropy quantity
    ...
    angle = -1 * np.arctan(
        (source_y - oa_y).value(source_x - oa_x).value) * u.rad << u.deg
    ...
    for i in ['1', '2']:
        ...
        map_res = nustar_map.meta[f'CDELT{i}'] * \
            u.Unit(nustar_map.meta[f'CUNIT{i}'])
        assert psf_res == map_res, 'The resolution in the PSF and the provided map are different.'

    assert -90*u.deg <= angle <= 90 * \
        u.deg, 'Angle between optical axis and source not within -90 to 90 deg. Invalid source and/or optical axis position?'
    psf_array = rotate(psf_array, angle.to(u.deg), reshape=True)
    deconv = restoration.richardson_lucy(
        nustar_map.data, psf_array, num_iter=iterations,
        clip=clip, filter_epsilon=1)
