...
class RegionSelector():
    '''Cut the data based on the region given to the object.'''
    ...
    def __init__(
        ...
        self._set_data_files(livetime_correction)
    ...
    @property
    def region_evts(self) -> np.ndarray | None:
        '''Pixel events contained within the region.'''
    ...
    def _param_str(self) -> str:
        '''String summarizing the parameters of the RegionSelector.'''
    ...
    def _read_data(self):
        ...
        self._full_evt_data, self.hdr = utilities.get_event_data(
            self.evt_file, self.time_range)
        self._energy_filter()

        if self.time_range is None:
            start = time_tools.nustar_to_astropy(
                self.evt_data['TIME'][0])
            end = time_tools.nustar_to_astropy(
                self.evt_data['TIME'][-1])
            self.time_range = (start, end)
            print('Time range not specified. Found time range:', start, end)

    def _energy_filter(self):
        '''Filter by the photon energies.'''
        inds = nustar.filter.by_energy(
            self._full_evt_data, *self.energy_range.value)
        self.evt_data = self._full_evt_data[inds]
        self._grade_filter()

    def _grade_filter(self):
        '''Filter by the photon event grades.'''
        inds = (self.evt_data['GRADE'] >= self.grade_range[0]) & (
            self.evt_data['GRADE'] <= self.grade_range[1])
        self.evt_data = self.evt_data[inds]

    def _make_map(self):
        '''Make the map using the event data.'''
        self._nustar_map = maps.make_nustar_map(self.evt_data, self.hdr)
        self._map_corners = list(mtools.find_min_max(
            self._nustar_map.data, 1)) * u.arcsec
    ...
    def _make_pixel_array(self):
        '''Create a PixelArray object from the region event data.'''
        if self.region is not None:
            map_ = self._region_map
        else:
            map_ = self._nustar_map
        self.pixel_array = pa.PixelArray(
            self.evt_data, self.hdr, map_=map_, region=self.region)
    ...
    def save_fig(self, fig_type: str):
    ...
    def plot_region(self, ax: plt.Axes, map_: sunpy.map.GenericMap, **kwargs):
        '''Plots the chosen region on the map and axes'''
        default_kwargs = {
            'color': 'lightblue',
            'edgecolor': None,
            'ls': 'dashed',
    ...
    def plot_deconvolved_map(
        ...
        deconv_data, psf_array = deconvolve(
            submap, psf_file, source_position, oa_position, iterations=iterations)
        submap.data[:] = deconv_data
        submap, fig, ax, _ = mtools.apply_map_settings(
            submap, fig=fig, index=index, **map_kw)
    ...
    def make_lightcurve(self, frame_length: float):
        '''Make a lightcurve with the specified frame_length, in seconds.'''
    ...
    def plot_inset_lightcurve(
        ...
        if fig is None:
            ...
            fig, ax, submap = self.plot_overview_map(
                region_kw=region_kw, map_kw=map_kw)
