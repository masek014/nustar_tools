'''
Defines classes for animating raw (detector) maps made from detector pixels.
'''
...
class RawPixelAnimation():
    '''Create a movie from the raw (i.e. detector) pixel data.'''
    ...
    def __init__(
        ...
        self.start, self.end, num_frames = utilities.characterize_frames(
            self.evt_data, time_step)
    ...
    def time_filter(self, time_range: tuple[Time, Time]) -> fits.FITS_rec:
        '''Returns the data, filtered by the provided time range.'''
        return self.evt_data[nustar.filter.by_time(self.evt_data, self.hdr, time_range)]

    def animate(self, out_dir: str = './', fps: float = 20, dpi: int = 400):
        '''Animate the movie.'''
        def update_mats(frame: int, self) -> list[np.ndarray]:
            '''Update the matrices for the next frame'''
            start_time = self.start + self.time_step*(frame)
            frame_start = time_tools.nustar_to_astropy(
                start_time)
        ...
        date = time_tools.nustar_to_datetime(
            self.start).strftime('%Y%m%d')
        file_name = f'{out_dir}/{date}_{self.id_num}_fpm{self.fpm}_'\
            f'{self.time_step}_{self.frame_length}_rawpixels.mp4'
        anim = animation.FuncAnimation(
            self.fig, update_mats, fargs=(self,),
            frames=self.end_frame, interval=1, blit=True)
...
class RawPixelAnimationCentroid(RawPixelAnimation):
    '''Create a movie from the centroid of the raw (i.e. detector) pixel data.'''
    ...
    def __init__(
        ...
        figsize = (4, 4)
        spacing = {
            'left': 0.03, 'bottom': 0.01, 'right': 0.97,
            'top': 0.95, 'wspace': 0.02, 'hspace': 0.02}
        rows = 3
        ratios.append(0.4)
        figsize = (4, 4.8)
        spacing = {
            'left': 0.05, 'bottom': 0.05, 'right': 0.95,
            'top': 0.95, 'wspace': 0.02, 'hspace': 0.02}
        self.fig = plt.figure(figsize=figsize)
        self.spec = self.fig.add_gridspec(
            rows, cols, height_ratios=ratios, **spacing)
        self.mats = []
    ...
    def animate(self, out_dir: str = './', fps: float = 20, dpi: float = 400):
        '''Create the movie.'''
        def init_animation():

            # init is called twice in FuncAnimation
            # this prevents double drawing the plots
            plt.clf()
            self.axs = []
            init_arrs = []
            for det in [0, 1, 2, 3]:
        ...
        def update_mats(frame, self):
            ...
            for det in range(det_arrs.shape[0]):
                ...
                self.mats[det].set_norm(None)
            line = self.mats[-1]
            dt = time_tools.nustar_to_datetime(start_time)
            x_centroid = photutils.centroids.centroid_com(
                np.fliplr(det_arrs[0]))[0]
        ...
        date = time_tools.nustar_to_datetime(
            self.start).strftime('%Y%m%d')
        file_name = f'{out_dir}/{date}_{self.id_num}_{self.time_step}_'\
            f'{self.frame_length}_rawpixels_centroid.mp4'
        anim = animation.FuncAnimation(
            self.fig, update_mats, init_func=init_animation,
