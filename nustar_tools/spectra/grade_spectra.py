...
def make_grade_distribution(
    ...
    if region_kwargs is not None:
        ...
        region = region_kwargs['Class'](
            SkyCoord(
                *region_kwargs['center'], frame=nustar_map.coordinate_frame),
    ...
    for grade in range(0, 32):  # Fill in missing grades.
...
class GradeSpectrum():
    ...
    def spectrum(self) -> tuple[u.Quantity, u.Quantity]:
        '''The grade spectrum data: values, edges'''
        values, edges, _ = pm.read_pha_spectrum(self.grade_file)
        return values, edges

    @property
    def label(self) -> str:
        '''A label for the grade spectrum.'''
    ...
    def plot(self, ax: plt.Axes = None) -> plt.Axes:
        '''Plot the grade spectrum.'''
        counts, edges = self.spectrum
        if ax is None:
            fig, ax = plt.subplots(figsize=(6, 6), layout='constrained')
        ax.stairs(counts.value, edges.value, label=self.label, **self.kwargs)
        ax.set(
            xlabel=f'Energy [{edges.unit}]', ylabel=f'Spectrum [{counts.unit}]')
...
class GradeCollection():
    ...
    def prepare_data(
        ...
        '''Prepare all the grade spectrum data.'''
        default_fpmA_kwargs = {'cmap': 'turbo'}
        default_fpmB_kwargs = {'cmap': 'plasma'}
        fpmA_kwargs = {**default_fpmA_kwargs, **fpmA_kwargs}
        fpmB_kwargs = {**default_fpmB_kwargs, **fpmB_kwargs}
        kwargs = {'A': fpmA_kwargs, 'B': fpmB_kwargs}
        self.data = {}
        ...
        for fpm in self.fpms:
            ...
            for grade in self.grades:
                grade_path = self.grade_path_format.format(
                    fpm=fpm, grade=grade)
                color = cmap(count/total_spectra)
                spectrum = GradeSpectrum(
                    grade_path, grade, fpm, {'color': color, **kwargs[fpm]})
    ...
    def plot_spectra(
        ...
        '''Plot the grade spectra in the collection.'''
        if ax is None:
                ...
                if pileup_correction:
                    ...
                    if '21-24' in self.data[fpm]:
                        if grade in PILEUP_GRADE_CORRECTIONS:
                            ...
                            kwargs = {**spectrum.kwargs,
                                      **default_pileup_kwargs}
                            kwargs = {**kwargs,
                                      **pileup_kwargs}
                            ax.stairs(
                                corrected.value, edges.value,
    ...
    def plot_ratios(
        ...
        '''Plot the ratio of the grade spectra referenced to the specified grade.'''
        if ax is None:
        ...
        for fpm in self.fpms:
            ...
            for grade in self.grades:
                ...
                ax.stairs(
                    ratio.value, edges.value, label=spectrum.label,
                    **spectrum.kwargs)
        ax.set(
            xlabel=f'Energy [{edges.unit}]', ylabel='Ratio',
