...
def read_pha_spectrum(
    ...
    with fits.open(pha_file) as hdu:
        data = hdu[1].data
        exposure = hdu[1].header['EXPOSURE'] * u.Unit(hdu[1].header['TIMEUNIT'])
    ...
    if minimum_counts_per_bin is not None:
        values, edges = minimum_bin_counts(
            values,
            edges,
            minimum_counts_per_bin
        )
    errors = np.sqrt( values.value + (additional_error_factor*values.value)**2 ) * u.ct
...
def make_thermal_pha(
    ...
    nonthermal_spectrum, nonthermal_edges = simulate_nonthermal_spectrum(
        handler.srm,
        handler.energy_edges,
        **nonthermal_parameters
    )

    # TODO: Figure out what to do with the factor!
    # if apply_fpm_scaling: # fpm == 'B':
    nonthermal_spectrum *= fpm_factor
    print('WARNING [make_thermal_pha]: The scaling factor between FPMs A and B needs to be accounted for. Fix this!!!')
    ...
    nonthermal_counts_phabins = nonthermal_spectrum_phabins * np.diff(pha_edges) * exposure
    thermal_pha_counts = np.trunc(pha_counts - nonthermal_counts_phabins)
    thermal_pha_counts[thermal_pha_counts < 0] = 0

    thermal_pha_counts_error = np.trunc( np.sqrt(pha_counts.value + nonthermal_counts_phabins.value) ) * u.ct

    shutil.copyfile(in_pha, out_pha)
    with fits.open(out_pha) as hdu:
        orig_header = hdu[1].header
        hdu[1].data['COUNTS'] = thermal_pha_counts
        new_col = fits.ColDefs([ fits.Column(name='STAT_ERR', format='J', array=thermal_pha_counts_error.value) ])
        ...
        hdu[1].header.set('POISSERR', 0, comment='Poissonian error not assumed')
        hdu[1].header.set('TTYPE3', 'STAT_ERR', comment='Count error per channel', after='TUNIT2')
        hdu[1].header.set('TFORM3', 'J', comment='data format of field: 4-byte INTEGER', after='TTYPE3')
        hdu[1].header.set('TUNIT3', 'count', comment='physical unit of field', after='TFORM3')
...
def make_pileup_corrected_pha(
    ...
    with fits.open(out_pha) as hdu:
        ...
        # so here we define a new CHANNEL column.
        orig_header = hdu[1].header
        new_col = fits.ColDefs([ fits.Column(name='COUNTS', format='J', array=corrected_counts) ])
        ...
        hdu[1].header.set(key, 'COUNTS', comment='Pileup-Corrected Counts per channel')
        hdu[1].header.set(f'TFORM{key[-1]}', 'J', comment='data format of field: 4-byte INTEGER')
